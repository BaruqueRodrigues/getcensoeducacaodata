...
pega_creches_municipio <- function(
){
  ...
  creches_municipio <- purrr::map(diretorio_csv,
                                  ~readr::read_delim(.x, delim = ";", escape_double = FALSE, locale = readr::locale(encoding = "WINDOWS-1252"),
                                                    # col_select = c("in_inf_cre", "no_uf", "no_municipio", "nu_ano_censo"),
                                                     trim_ws = TRUE) %>%
                                    janitor::clean_names() %>%
                                    dplyr::filter(in_inf_cre == 1) %>%
                                    dplyr::count(no_uf,no_municipio, nu_ano_censo,name = "total_creches")
  ) %>%
    purrr::list_rbind()
