#' Pega Ensino Médio por Município
#'
...
pega_ensino_medio_municipio <- function(
){
  ...
  tabela_educacao_mun <- purrr::map(diretorio_csv,
                                    ~readr::read_delim(.x, delim = ";", escape_double = FALSE, locale = readr::locale(encoding = "WINDOWS-1252"),
                                                       #col_select = c("in_med", "no_uf", "no_municipio", "nu_ano_censo"),
                                                       trim_ws = TRUE) %>%
                                      janitor::clean_names() %>%
                                      dplyr::filter(in_med == 1) %>%
                                      dplyr::count(no_uf,no_municipio, nu_ano_censo,name = "total_ensino_medio")
  ) %>%
    purrr::list_rbind()
